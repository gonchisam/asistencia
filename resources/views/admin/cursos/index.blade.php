<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Gestión de Cursos (Grupos)
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    
                    <div class="flex justify-between items-center mb-4">
                        <x-primary-button onclick="location.href='{{ route('admin.cursos.create') }}'">
                            Crear Nuevo Curso
                        </x-primary-button>
                        <x-secondary-button onclick="location.href='{{ route('admin.inscripciones.importar.vista') }}'">
                            Importar Inscripciones (Excel)
                        </x-secondary-button>
                    </div>

                    @include('admin.partials._session-messages') <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Materia</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Carrera</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Año</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Paralelo</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Gestión</th>
                                    <th class="relative px-6 py-3"><span class="sr-only">Acciones</span></th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($cursos as $curso)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap font-medium">{{ $curso->materia->nombre }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $curso->materia->carrera }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $curso->materia->ano_cursado }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $curso->paralelo }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $curso->gestion }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('admin.cursos.show', $curso) }}" class="text-blue-600 hover:text-blue-900">Gestionar</a>
                                            <a href="{{ route('admin.cursos.edit', $curso) }}" class="text-indigo-600 hover:text-indigo-900 ml-2">Editar</a>
                                            </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-4 whitespace-nowrap text-center text-gray-500">No hay cursos registrados.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    <div class="mt-4">
                        {{ $cursos->links() }}
                    </div>
                </div>
            </div>
        </div>
    </div>
</x-app-layout>